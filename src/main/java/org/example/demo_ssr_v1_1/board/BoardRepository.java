...
public interface BoardRepository extends JpaRepository<Board, Long> {
     ...
     *
     * countQuery 를 적지 않으면 Spring Data 가 메인 쿼리에서 SELECT 절을 떼고 COUNT 로 바꿔
     * 자동 생성을 시도한다. 하지만 JOIN FETCH 같은 복잡한 쿼리에서는 자동 변환이 실패하거나
     * 불필요하게 user 테이블까지 JOIN 해서 비효율적이다. 그래서 명시적으로 적어 준다.
     *
     * 비유: 책장에서 책을 꺼낼 때, "이번에 보여줄 책 5권을 꺼내는 일" 과 "이 책장에 총 몇 권이
     * 꽂혀 있는지 세는 일" 은 별개다. 두 번째 작업에는 굳이 책 표지를 펴서 작가 정보까지 볼
     * 필요가 없다. 그래서 countQuery 에는 JOIN FETCH 를 안 넣는다.
     *
     * [DISTINCT 가 필요한 이유]
